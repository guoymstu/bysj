    ...
    <div class="handle-box">
        <ul>
            <li class="handle-item"><span class="layui-form-label">输入课程名或编号：</span>
                ...
                <select id="status" class="layui-input layui-input-inline" name="status" lay-verify="">
                <option value="" selected="selected">全部</option>
                <option value="1">有余量</option>
                </select>
                </div>
                ...
                <%--<div class="fr">--%>
                    ...
                    <%--&lt;%&ndash;<a class="layui-btn btn-add btn-default" id="btn-addstu"><i class="linyer icon-add"></i> 添加学生</a>&ndash;%&gt;--%>
                <%--</div>--%>
            </li>
        </ul>
    </div>
    ...
    layui.use(['layer', 'datatable', 'datatableButton', 'datatableFlash', 'datatableHtml5', 'datatablePrint', 'datatableColVis', 'datatableSelect'], function () {
        ...
        $(document).ready(function () {
            var myTable = $('#courseTable').DataTable({
                ...
                "ajax": {
                    url: 'getCourse.shtm?keyword=' + $("#keyword").val()+'&status='+$("#status").val(),
                    dataSrc: 'courses'
                },
                ...
                "columns": [
                    ...
                    {
                        "data": function (obj) {
                            return obj.countstu-obj.countsel;
                        },
                        "sTitle": "课程余量", //标题
                        "sType": 'chinese',
                        "sDefaultContent": "", //此列默认值为""，以防数据中没有此值，DataTables加载数据的时候报错
                    },
                    {
                        "data": function(obj){
                            if(obj.status=="1"){
                                return '<span class="label label-success radius">可选择</span>';
                            } else {
                                return '<span class="label label-default radius">已满</span>';
                            }
                        },
                        "className": "td-status",
                        "sTitle": "状态", //标题
...

                    {
                        "data": function (obj) {
                            if (obj.status == "1") {
                                return '<button class="layui-btn handle-select">选择</button>' ;
                            } else {
                                return  '<button class="layui-btn layui-btn-disabled">选择</button> ';
                            }
                        },
                        "className": "td-handle",
                        "sTitle": "操作", //标题
            ...
            function reloadTable() {
                ...
                myTable.ajax.url('getCourse.shtm?keyword=' + $("#keyword").val()+'&status='+$("#status").val()).load();
            }
        ...
        /*学生选课*/
        $('.table-sort').on('click', '.handle-select', function () {
            var obj = $(this);
            ...
            layer.confirm('选修此课吗？', {
                ...
                $.ajax({
                    async: false,
                    url: '/student/course/selectCourse.shtm',
