...
<body>
<div class="tab-body"><p class="page-tab"><span class="layui-breadcrumb" lay-separator=">"><a href="">首页</a><a><cite>用户列表</cite></a></span>
</p>
        ...
        <ul>
            <li class="handle-item">
                ...
                <div style="float: left" class="layui-input-inline">
                    <select id="role" class="layui-input layui-input-inline" name="courseType" lay-verify="">
                        <option value="teacher" selected="selected">艺术实践课</option>
                ...
                <div class="layui-input-inline">
                    <select id="role" class="layui-input layui-input-inline" name="pjtype" lay-verify="">
                        <option value="teacher" selected="selected">教学态度</option>
                        <option value="student">学生</option>
                    </select>
                </div>
                ...
                <div class="fr">
                    ...
                    <a class="layui-btn btn-add btn-default" id="btn-addpingjiao"><i class="linyer icon-add"></i>
                        添加评教</a>
                </div>
            </li>
        </ul>
    ...
    layui.use(['layer', 'datatable', 'datatableButton', 'datatableFlash', 'datatableHtml5', 'datatablePrint', 'datatableColVis', 'datatableSelect'], function () {
        ...
        $(document).ready(function () {
            var myTable = $('#userTable').DataTable({
                ...
                "aoColumnDefs": [{
                    "orderable": false,
                    "aTargets": [0, 7] // 指定列不参与排序
                }],
                "deferRender": true, //延迟渲染
                "ajax": {
                    url: 'getPingjiao.shtm?keyword=' + $("#keyword").val() + '&coursetype=' + $("#pjtype").val(),
                    dataSrc: 'pingjiaos'
                },
            ...
            function reloadTable() {
                ...
                myTable.ajax.url('getPingjiao.shtm?keyword=' + $("#keyword").val() + '&role=' + $("#pjtype").val()).load();
            }
        ...
        /*添加-评教*/
        $("#btn-addpingjiao").on('click', function () {
            var title = $(this).html();
            var href = 'addPingjiaoForm.shtm';
        ...
        $('.table-sort').on('click', '.handle-btn-stop', function () {
            ...
            layer.confirm('确认要停用吗？', {
                ...
                $.ajax({
                    async: false,
                    url: '/admin/user/blockuser.shtm',
                    data: {useruid: useruid, status: 1},
                    type: 'post',
                    dataType: "json",
        ...
        $('.table-sort').on('click', '.handle-btn-delect', function () {
            var obj = $(this);
            var obj = $(this);
            var pingjiaoid = $(obj).parents("tr").find("td:nth-child(2)").html();
            console.log(pingjiaoid);
            layer.confirm('确认要删除吗？', {
                ...
                $.ajax({
                    async: false,
                    url: '/admin/pingjiao/delPingjiao.shtm',
                    data: {pingjiaoid: pingjiaoid},
                    type: 'post',
                    dataType: "json",
                    success: function (data) {
                        if (data.status == 'success') {
